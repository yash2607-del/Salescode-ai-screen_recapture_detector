# Screen Recapture Detector (Deep Learning)

A robust Convolutional Neural Network (CNN) pipeline leveraging Transfer Learning to detect presentation attacks (fraudsters taking photos of digital screens).

## Problem
Identity verification systems are vulnerable to presentation attacks where malicious actors photograph a digital screen displaying a stolen ID or deepfake. This project combats identity fraud by detecting the visual artifacts generated by physical screens (such as Moiré patterns, glare, and pixel grids).
...
- **Model Loading**: ~1.5s
- **Preprocessing**: < 50ms
- **Inference Latency**: ~60-150ms per image (CPU).
